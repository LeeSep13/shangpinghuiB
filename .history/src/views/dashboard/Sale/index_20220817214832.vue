 ...
 * @LastEditTime: 2022-08-17 21:48:32
      ...
      <div style="width:25%">
        ...
        <ul v-if="this.activeName == 'sale'">
          <li v-for="(store, index) in storeranking.sale" :key="index">
            <!-- circle -->
            <span class="rindex" :class="{ circle: index < 3 }">{{ store.no }}</span>
  ...
  watch: {
    title() {
      if (this.activeName === 'sale') {
        this.setSaleCharts();
        this.storeranking.sale = this.listState.orderRank
      } else {
        // 重新修改图表的配置数据
        // 图表配置数据可以再次修改，如果有新的数值用新的，没有新的话用旧数据
        this.setVisitsCharts();
        this.storeranking.visits = this.listState.userRank
      }
    },
    listState() {
      this.setSaleCharts();
